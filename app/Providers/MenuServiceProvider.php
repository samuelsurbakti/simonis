...
class MenuServiceProvider extends ServiceProvider
{
    ...
    {
        app('view')->composer('layouts.simonis.app', function($view) {
            if(app('request')->route()) {
                $action = app('request')->route()->getAction();
                $exploid = 'Controllers\Simonis\\';
                ...
                $action = app('request');
                $menus = Menu::withCount('get_child')->where('role', Auth::user()->getRoleIds()->first())->whereNull('parent')->orderBy('no', 'asc')->get();
                $page_title = 'Error';
                $controller = 'Error';
